...
# Keys are generated only once per run
...
def compare_ips_and_ttl(results):
    ...
    subnet_safe = all(
        same_subnet(all_ips[i], all_ips[j])
        for i in range(len(all_ips)) for j in range(i+1, len(all_ips))
    )
...
for domain in domains:
    ...
    # Generate Verdict
    verdict = compare_ips_and_ttl(results)
